...
echo -n "Create ADMIN ACCOUNT? (y/n) "
read yesno < /dev/tty

if [ "x$yesno" = "y" ];then
   # Yes
   echo "Now we will create new account, please fill info"
   read -p "Admin Name=: " ADMNAME
   echo "Data received"
   read -p "Admin Email=: " ADMEMAIL
   echo "Data received"
   read -p "Admin Password=: " ADMPASS
   echo "Data received"
   read -p "This user is ADMIN or USER ?=: " TYPEOFUSER
   echo "Data received"
   docker exec greenlight-v2 bundle exec rake user:create["$ADMNAME","$ADMEMAIL","$ADMPASS","$TYPEOFUSER"]
else

   # No
   echo "Ok, you not create account.!"
fi



echo -n "Hack Greenlight (y/n) "
read yn < /dev/tty
...
if [ "x$yn" = "y" ];then
  ...
  sed -i 's/image: bigbluebutton/greenlight:v2/image: bigbluebutton/greenlight:release-v2/g' docker-compose.yml
  ...
  bbb-conf --clean
   ...
   # No
   echo "Ok, you Using default version of greenlight.!"
fi

echo -n "Wating (y/n), after do done. press N to coutinue "
read yesno < /dev/tty
